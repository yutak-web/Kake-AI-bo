...
import { getWallets, getTransactions, getCategories, deleteTransaction, updateTransaction } from '../services/db';
...
const WalletDetailsPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    setTransactions(t.filter(tx => tx.fromWalletId === walletId || tx.toWalletId === walletId));
    setCategories(c);
  };
  ...
  if (!wallet) return <div className="flex justify-center items-center h-64 text-gray-400">読み込み中...</div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 pb-10">
      <div className="flex items-center space-x-2 text-sm text-gray-500">
        <Link to="/aggregation" className="hover:underline">集計</Link>
        <span>&gt;</span>
      ...
      <div className="text-center">
        ...
        <p className="text-xl mt-2 text-gray-600 font-mono">現在の残高: ¥{currentBalance.toLocaleString()}</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="h-80 sketch-border p-4 bg-white shadow-sm">
          <h4 className="text-sm font-bold text-gray-400 mb-4 flex justify-between items-center">
            <span>残高推移 ({PERIODS.find(p => p.days === periodDays)?.label})</span>
            <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded text-gray-500 uppercase">Daily Final Balance</span>
          </h4>
          <div className="h-[230px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                />
                <Tooltip 
                  contentStyle={{ borderRadius: '8px', border: '2px solid #333', fontSize: '12px' }} 
      ...
      <div className="space-y-4">
        <h4 className="font-bold border-b border-gray-200 pb-2 flex justify-between items-center">
          <span>履歴一覧</span>
          <span className="text-xs font-normal text-gray-400">{transactions.length} 件</span>
        </h4>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {[...transactions].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(tx => {
                ...
                const otherWallet = allWallets.find(w => w.id === otherWalletId);
                return (
                  <tr key={tx.id} className="hover:bg-gray-50 transition-colors group">
                    <td className="px-2 py-4 whitespace-nowrap">
                      <div className="text-gray-500 text-[10px]">{tx.date}</div>
                      <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold mt-1 ${tx.type === 'income' ? 'bg-green-100 text-green-800' : tx.type === 'expense' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                        {tx.type === 'income' ? '収入' : tx.type === 'expense' ? '支出' : '移動'}
                      </span>
                    </td>
                    <td className="px-2 py-4">
                      <div className="flex items-center space-x-1 mb-1">
                        {tx.type !== 'transfer' && cat?.color && <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: cat.color }} />}
                        <div className="text-[10px] text-gray-400 font-bold uppercase truncate max-w-[120px]">{tx.type === 'transfer' ? '資金移動' : (cat?.name || '未設定')}</div>
                      </div>
                      ...
                      <div className="text-[9px] text-gray-500 bg-gray-100 inline-block px-1 rounded">
                        {tx.type === 'transfer' ? <span>{isOut ? `→ ${otherWallet?.name || '不明'}` : `← ${otherWallet?.name || '不明'}`}</span> : <span>{isOut ? `支払財布: ${wallet.name}` : `入金財布: ${wallet.name}`}</span>}
                      </div>
                    </td>
                    <td className={`px-2 py-4 text-right font-bold font-mono whitespace-nowrap ${isOut ? 'text-red-600' : 'text-green-600'}`}>
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-2 py-8 text-center text-gray-400 italic">取引履歴がありません</td>
                </tr>
              )}
            </tbody>
          </table>
